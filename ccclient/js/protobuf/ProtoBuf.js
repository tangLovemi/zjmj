...
(function(global) {
    ...
    function loadProtoBuf(ByteBuffer) {
        ...
        ProtoBuf.Util = (function() {
             ...
             * @expose
             */
            Util.fetch = function(path, callback, needDecrypt) {
                if (callback && typeof callback != 'function') callback = null;
                var t = sys.platform;
                if("browser" != t){
                    Log("Util.fetch: on mobile");
                    var text = jsclient.FileUtils.getStringFromFile(path, needDecrypt);
                    if(callback){
                        callback(text);
                        return;
                    }
                                ...
                                if (def["fields"] && def["fields"].length > 0) {
                                    for (i=0; i<def["fields"].length; i++) { // i=Fields
                                        ...
                                        if (def["fields"][i]["options"]) {
                                            ...
                                            for (j=0; j<subObj.length; j++) { // i=Option names
