(function (exports, ByteArray, global) {
    ...
    Message.encode = function (id, type, compressRoute, route, messageId, msg) {
        ...
        msgLen += caculateMsgIdBytes(messageId); //new add
        ...
        // add message id
        offset = encodeMessageId(messageId, buffer, offset); //new add
    ...
    Message.decode = function (buffer) {
        ...
        //parse message id  (new add)
        {
            var m = parseInt(bytes[offset]);
            var i = 0;
            do {
                var m = parseInt(bytes[offset]);
                messageId = messageId + ((m & 0x7f) * Math.pow(2, (7 * i)));
                offset++;
                i++;
            } while (m >= 128);
        }
